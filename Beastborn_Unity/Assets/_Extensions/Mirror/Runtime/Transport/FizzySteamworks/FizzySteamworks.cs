    ...
    public override void ServerStart()
    {
      ...
      if (!ServerActive())
      {
        if (UseNextGenSteamNetworking)
        {
          ...
          server = NextServer.CreateServer(this, NetworkManagerExtended.singleton.maxConnections);
        }
        else
        {
          Debug.Log($"Starting server [DEPRECATED SteamNetworking]. Relay enabled: {AllowSteamRelay}");
          SteamNetworking.AllowP2PPacketRelay(AllowSteamRelay);
          server = LegacyServer.CreateServer(this, NetworkManagerExtended.singleton.maxConnections);
        }
      }
      else
      {
