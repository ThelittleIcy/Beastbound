...
namespace BeastbornNetwork
{
    ...
    [AddComponentMenu("Network/NetworkManagerExtended")]
    ...
    public class NetworkManagerExtended : MonoBehaviour
    {
        ...
        public int maxVRPlayerAmount = 1;
        List<NetworkConnection> VRPlayerConnectionList = new List<NetworkConnection>(); 

        [Tooltip("Prefab of the player object. Prefab must have a Network Identity component. May be an empty game object or a full avatar.")]
        public GameObject playerPrefabFS;

        public int maxFSPlayerAmount = 1;
        List<NetworkConnection> FSPlayerConnectionList = new List<NetworkConnection>(); 
        ...
        public static NetworkManagerExtended singleton { get; private set; }
        ...
        {
            //playerPlaceHolderPrefab = playerPrefabVR;
            ...
            // always >= 0
            maxFSPlayerAmount = Mathf.Max(maxFSPlayerAmount, 0);
            maxVRPlayerAmount = Mathf.Max(maxVRPlayerAmount, 0);

            maxConnections = maxVRPlayerAmount + maxFSPlayerAmount;

            if (playerPrefabVR != null && playerPrefabVR.GetComponent<NetworkIdentity>() == null)
            {
                Debug.LogError("NetworkManager - VR Player Prefab must have a NetworkIdentity.");
                playerPrefabVR = null;
            }

            // This avoids the mysterious "Replacing existing prefab with assetId ... Old prefab 'Player', New prefab 'Player'" warning.
            if (playerPrefabVR != null && spawnPrefabs.Contains(playerPrefabVR))
            {
                Debug.LogWarning("NetworkManager - VR Player Prefab should not be added to Registered Spawnable Prefabs list...removed it.");
                spawnPrefabs.Remove(playerPrefabVR);
            }

            if (playerPrefabFS != null && playerPrefabFS.GetComponent<NetworkIdentity>() == null)
            {
                Debug.LogError("NetworkManager -  FlatScreen Player Prefab must have a NetworkIdentity.");
                playerPrefabFS = null;
            }

            // This avoids the mysterious "Replacing existing prefab with assetId ... Old prefab 'Player', New prefab 'Player'" warning.
            if (playerPrefabFS != null && spawnPrefabs.Contains(playerPrefabFS))
            {
                Debug.LogWarning("NetworkManager - FlatScreen Player Prefab should not be added to Registered Spawnable Prefabs list...removed it.");
                spawnPrefabs.Remove(playerPrefabFS);
            }
        }
...
#pragma warning restore 618
            ...
            VRPlayerConnectionList?.Clear();
            FSPlayerConnectionList?.Clear();
        }
...
#pragma warning restore 618
        ...
        void RegisterServerMessages()
        {
            ...
            NetworkServer.RegisterHandler<AddVRPlayerMessage>(OnServerAddVRPlayerInternal);
            NetworkServer.RegisterHandler<AddFSPlayerMessage>(OnServerAddFSPlayerInternal);
        ...
        void RegisterClientMessages()
        {
            ...
            NetworkClient.RegisterHandler<ConnectionGotDeclinedLobbyFullMessage>(OnConnecntionGotDeclined, false);

            if (playerPrefabVR != null)
                NetworkClient.RegisterPrefab(playerPrefabVR);

            if (playerPrefabFS != null)
                NetworkClient.RegisterPrefab(playerPrefabFS);
        ...
        {
            Debug.LogError("You are Trying to add a Player, but not specifiy what kind of Player (VR or FS). This will not work!");
            conn.Send(new ConnectionGotDeclinedLobbyFullMessage());       
        }

        void OnServerAddVRPlayerInternal(NetworkConnection conn, AddVRPlayerMessage msg)
        {
            Debug.Log("New VR Player");

            if (autoCreatePlayer && playerPrefabVR == null)
            {
                Debug.LogError("The PlayerPrefab is empty on the NetworkManager. Please setup a PlayerPrefab object.");
                return;
            }

            if (autoCreatePlayer && playerPrefabVR.GetComponent<NetworkIdentity>() == null)
            {
            ...
            if(VRPlayerConnectionList == null)
            {
                VRPlayerConnectionList = new List<NetworkConnection>();
            }

            if(VRPlayerConnectionList.Count >= maxVRPlayerAmount)
            {
                Debug.LogWarning("There are already the max amount of VR Players connected!");
            ...
            VRPlayerConnectionList.Add(conn);

            OnServerAddVRPlayer(conn);
        }

        void OnServerAddFSPlayerInternal(NetworkConnection conn, AddFSPlayerMessage msg)
        {
            Debug.Log("New FlatScreen Player");
            ...
            if (FSPlayerConnectionList == null)
            {
                //Debug.Log("New FS List");
                FSPlayerConnectionList = new List<NetworkConnection>();
            }

            if (FSPlayerConnectionList.Count >= maxFSPlayerAmount)
            {
                Debug.LogWarning("There are already the max amount of FlatScreen Players connected!");
                conn.Send(new ConnectionGotDeclinedLobbyFullMessage());
                return;
            }

            FSPlayerConnectionList.Add(conn);

            OnServerAddFSPlayer(conn);
        }
        ...
        void OnConnecntionGotDeclined(ConnectionGotDeclinedLobbyFullMessage msg)
        {
            Debug.LogWarning($"The Lobby was allready full with {platform} players!");
            StopClient(); 
        }
        ...
        public virtual void OnServerDisconnect(NetworkConnection conn)
        {
            ...
            if (VRPlayerConnectionList.Contains(conn))
            {
                VRPlayerConnectionList.Remove(conn);
            }
            
            else if (FSPlayerConnectionList.Contains(conn))
            {
                FSPlayerConnectionList.Remove(conn);
            }
        ...
        public virtual void OnServerAddVRPlayer(NetworkConnection conn)
        {
            Transform startPos = GetStartPosition();

            GameObject player = startPos != null
                ? Instantiate(playerPrefabVR, startPos.position, startPos.rotation)
                : Instantiate(playerPrefabVR);

            // instantiating a "Player" prefab gives it the name "Player(clone)"
            // => appending the connectionId is WAY more useful for debugging!
            player.name = $"{playerPrefabVR.name} [connId={conn.connectionId}]";
            NetworkServer.AddPlayerForConnection(conn, player);
        }

        public virtual void OnServerAddFSPlayer(NetworkConnection conn)
        {
            Transform startPos = GetStartPosition();

            GameObject player = startPos != null
                ? Instantiate(playerPrefabFS, startPos.position, startPos.rotation)
                : Instantiate(playerPrefabFS);

            // instantiating a "Player" prefab gives it the name "Player(clone)"
            // => appending the connectionId is WAY more useful for debugging!
            player.name = $"{playerPrefabFS.name} [connId={conn.connectionId}]";
            NetworkServer.AddPlayerForConnection(conn, player);
        }
        ...
        public virtual void OnClientConnect(NetworkConnection conn)
        {
            ...
            if (!clientLoadedScene)
            {
                ...
                if (autoCreatePlayer)
                {
                    ...
                    {
                        case EPlatform.VR:
                            NetworkClient.AddVRPlayer();
                            break;
                        case EPlatform.FS:
                            NetworkClient.AddFSPlayer();
                            break;
                    }
        ...
        public void SpawnPlayerPrefabForPlayer(NetworkIdentity networkIdentity, EPlatform _platform)
        {
            GameObject go = null;
            ...
            {
                case EPlatform.VR:
                    go = Instantiate(playerPrefabVR);
                    //NetworkServer.Spawn(go);
                    NetworkServer.AddPlayerForConnection(networkIdentity.connectionToClient, go);
                    break;
                    
                case EPlatform.FS:
                    go = Instantiate(playerPrefabFS);
                    //NetworkServer.Spawn(go);
                    NetworkServer.AddPlayerForConnection(networkIdentity.connectionToClient, go);
                    break;
            }
        ...
        public void SetBuild(EPlatform buildValue)
        {
            platform = buildValue;
        }
    }
